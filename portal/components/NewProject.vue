    ...
    .newRow
        table.table
            tr.header
                ...
                th.target Target
                th
            tr(v-for="(project, parent) in projects")
                td
                    input.infoInput.savedDisable(value="" v-model="project.wordcount" readonly)
                td
                    input.infoInput(value="" v-model="project.key" :readonly="project.readonly" :class='{savedDisable: projects[main].icons[0].status}')
                td
                    input.infoInput(value="" v-model="project.summary.text" :readonly="project.readonly" :class='{errorActive: !project.summary.exist && !project.summary.text, savedDisable: projects[main].icons[0].status}')
                td.lang-options
                    .select
                        target-lang(
                            :projects="projects"
                ...
                td
                    input.infoInput(value="" v-model="project.source.text" :readonly="project.readonly" :class='{errorActive: !project.source.exist && !project.source.text, savedDisable: projects[parent].icons[0].status}')
                td
                    input.infoInput(value="" v-model="project.target.text" :readonly="project.readonly" :class='{errorActive: !project.target.exist && !project.target.text, savedDisable: projects[main].icons[0].status}')
                td.icons
                    span.icons__item(v-for="(icon, iconIndex) in project.icons" :class="{[`disabled-${iconIndex + 1}`]: icon.status }")
                        img(:src='icon.icon' @click="projectAction(main, iconIndex)")
</template>
