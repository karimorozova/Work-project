	...
	export default {
		...
		methods: {
			...
			setLangFilter({ option }, prop) {
				const lang = this.allLanguages.find(item => item.lang === option);
				const filter = prop === 'source' ? 'sourceFilter' : 'targetFilter';
				this.$emit("setLangFilter", { value: lang.symbol, filter })
			},
		...
		computed: {
			...
			sourceLangs() {
				return this.clientSourceLanguages && this.clientSourceLanguages.map(item => item.lang).filter((item, index, arr) => {
					return arr.indexOf(item) === index
				})
			},
			targetLangs() {
				return this.clientTargetLanguages && this.clientTargetLanguages.map(item => item.lang).filter((item, index, arr) => {
					return arr.indexOf(item) === index
				})
			}
		},
