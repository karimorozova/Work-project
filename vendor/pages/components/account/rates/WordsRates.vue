<template lang="pug">
    .duo-rates
        ...
        .duo-rates__rates
            DuoRatesTable(:rates="sortedRates" :sourceFilter="sourceSelect" :targetFilter="targetSelect" :industriesFilter="industriesNames")
</template>
...
export default {
    ...
    methods: {
        ...
        setServiceFilter({option}) {
            ...
            if(!this.selectedSteps.length) return this.selectedSteps = ["Translation"];
        },
        setDefaultStep() {
            const defaultStep = this.steps.find(item => item.symbol === 'translation');
    ...
    computed: {
        ...
        sourceLanguages() {
            let result = [];
            const { wordsRates } = this.accountInfo;
            if(wordsRates.length) {
                result = wordsRates.filter(item => item.source).map(item => item.source.lang);
        ...
        targetLanguages() {
            ...
            const { wordsRates } = this.accountInfo;
            if(wordsRates.length) {
                result = wordsRates.map(item => item.target.lang);
                result = result.filter((item, i, arr) => arr.indexOf(item) === i);
                result.sort((a, b) => a > b ? 1 : -1);
                result.unshift("All");
            }
        ...
        stepsNames() {
            ...
            if(this.steps.length) {
                result = this.steps.filter(item => item.calculationunit === "Words")
                .map(item => item.title);
            }
            return result;
        },
        sortedRates() {
            let result = [...this.accountInfo.wordsRates];
            if(result.length) {
