  ...
  .drop-select(v-click-outside="outClick")
    .select
      template(v-if="selectedInd")
        .selected {{ selectedInd.invoiceDate}}
      template(v-if="!selectedInd")
    ...
    .drop(v-if="droppedInd")
      .drop__item(v-for="(job, index) in uniqJobInvoiceDates" @click="changeInd(index)" :class="{chosen: job.invoiceDate=== selectedInd.invoiceDate}")
        span {{ job.invoiceDate}}
</template>
