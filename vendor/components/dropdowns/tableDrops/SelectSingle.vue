<template lang="pug">
    .drop-select(v-click-outside="outOptions" :class="{'z-main': isDropped}")
        .select
        ...
        .drop(v-if="isDropped")
            .drop__item(v-for="(option, parent) in options" @click="chooseOption(parent)" :class="{active: selectedOption == option}")
                span {{ option }}
</template>
