...
const fillLangTierReportWithLocal = async () => {
	...
	let finishedProjects = await Projects.find({ $and: [{ status: 'Closed' }, { isTest: false }, { startDate: { '$gte': sixMonthsAgo } }] })
	...
	for (let { steps, industry } of finishedProjects) {
		...
		function recalculateWordcount(reportIndex, sourceLanguage, targetLanguage, finance) {
			...
			if(sameSourceIndex === -1) {
				langTierReports[reportIndex].source.push({ lang: source.group, targets: [{ wordcount: finance.Wordcount.receivables, lang: target.group }] })
			} else {
				const sameTargetIndex = langTierReports[reportIndex].source[sameSourceIndex].targets.findIndex(i => i.lang === target.group);
				if(sameTargetIndex === -1) {
					langTierReports[reportIndex].source[sameSourceIndex].targets.push({ wordcount: finance.Wordcount.receivables, lang: target.group })
				} else {
