...
const getUpdatedPricelistRow = (basicPricesTable, stepMultipliersTable, industryMultipliersTable, objToBind) => {
	const { basicPrice } = basicPricesTable.find(({ sourceLanguage, targetLanguage }) => (
			sourceLanguage === objToBind.sourceLanguage &&
			targetLanguage === objToBind.targetLanguage
	))
	const { multiplier: stepMultiplierValue } = stepMultipliersTable.find(({ step, unit }) => (
			step === objToBind.step &&
			unit === objToBind.unit
	))
	...
	objToBind.price = multiplyPrices(basicPrice, stepMultiplierValue, industryMultiplierValue)
	return objToBind
}
