...
.tasks-langs
  .source
    .tasks-langs__title Source Language:
...
export default {
  ...
  methods: {
    ...
    setSource({ option }) {
      const { symbol, lang } = this.originallyLanguages.find((item) => item.lang === option);
      this.$emit("setSourceLanguage", { symbol: symbol });
      this.setPossibleTargets(symbol);
      this.targetChosen = [];
    },

    setPossibleTargets(symbol) {
      this.targetAll = this.originallyLanguages.filter((item) =>
        this.currentProject.customer.targetLanguages.some((id) => item._id.toString() === id)
      );
    ...
    setUniqueLangs(arrProp) {
      this[arrProp] = this[arrProp].reduce(
        (acc, cur) => (acc.findIndex((el) => el._id === cur._id) < 0 ? [...acc, cur] : acc),
        []
      );
    },
    emitTargets() {
  ...
  computed: {
    ...
    possibleSourceLanguages() {
      if (this.currentProject._id) {
        return this.originallyLanguages
          .filter((item) => this.currentProject.customer.sourceLanguages.includes(item._id))
          .map((item) => item.lang);
      }
    },
  },
