<template lang="pug">
  .tasks-langs
    .source(v-if="currentProject.requestForm.service.languageForm === 'Duo'")
      .tasks-langs__title-source Source Language:
      .source__drop-menu
        SelectSingle(
          placeholder="Option",
          :options="['mappedSourceLanguages']",
          :selectedOption="currentProject.requestForm.sourceLanguage.lang",
          @chooseOption=""
          :isDisabled="true"
        )
...
export default {
  methods: {
    ...
    getServiceTargetLanguages() {
      const { requestForm: { targetLanguages } } = this.currentProject
