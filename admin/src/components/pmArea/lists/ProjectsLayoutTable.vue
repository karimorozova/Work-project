...
export default {
  ...
  methods: {
    ...
    progress(project) {
      ...
      const steps = project.steps.filter(({ status }) => status !== 'Cancelled' && status !== 'Cancelled Halfway')
      ...
      // 		.map(({ title }) => title)
      //
      // const tasks = project.tasks.filter(({ status }) => status !== 'Cancelled' && status !== 'Cancelled Halfway')
      //
      // tasks.forEach(task => {
      // 	let taskSteps = project.steps.filter(({ taskId }) => taskId === task.taskId)
      // 	taskSteps = taskSteps.filter(({ stepId }) => !stepId.includes('Cancelled'))
      //
