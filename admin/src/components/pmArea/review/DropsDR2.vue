  ...
  .drops
    DataTable(
      ...
      .drops__data(slot="dr1" slot-scope="{ row }")
        .drops__name {{ `${ dr1Manager.firstName } ${ dr1Manager.lastName }` }}
          .drops__timestamp(v-if="true")
            img.drops__time-icon(src="../../../assets/images/time_icon.png")
            .drops__time-data time
        //.drops__menu(v-else)
          SelectSingle(:options="managersNames":selectedOption="selectedDr1Manager"@chooseOption="(e) => setManager(e, 'dr1Manager')")
	...
	export default {
		...
		data() {
			return {
				...
				selectedContacts: [],
				fields: [
					{ label: "Delivery Review 1", headerKey: "headerDr1", key: "dr1", width: "33.3%", padding: 0 },
					{ label: "Delivery Review 2", headerKey: "headerDr2", key: "dr2", width: "33.4%", padding: 0 },
					{ label: "Contacts", headerKey: "headerContacts", key: "contacts", width: "33.3%", padding: 0 }
				]
			}
		},
		methods: {
			...
			getDr1Name() {
				...
				// if (this.timestamp) {
					return `${ this.dr1Manager.firstName } ${ this.dr1Manager.lastName }`
				// }
			},
			// getDeliveredTime() {
