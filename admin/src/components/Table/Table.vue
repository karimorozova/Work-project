      ...
      .b-table__body.b-table__display
        .b-table__row(v-for='(fullRowInfo, rowIndex) in table.body')
            .b-table__col.higher(v-for='(row, index) in fullRowInfo.rowInfo' :class='["b-table__col-" + (index + 1), {set_bottom_border: index == 0 || index == 1 || index == 2 || index == 3 || index == 4 || index == 5 || index == 6}]') {{row.title}} 
              ...
              input.clazz1(v-if="index == 1" type="text" v-model="row.title" :readonly="!fullRowInfo.isActiveUpload" @onEdit='editRow(rowIndex)')
              input.clazz2(v-if="index == 2" type="text" v-model="row.title" :readonly="!fullRowInfo.isActiveUpload" @onEdit='editRow(rowIndex)')
              input.clazz3(v-if="index == 3" type="text" v-model="row.title" :readonly="!fullRowInfo.isActiveUpload" @onEdit='editRow(rowIndex)')
