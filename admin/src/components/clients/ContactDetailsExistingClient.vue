...
<script>
	...
	export default {
		...
		methods: {
			...mapActions({
				updateActionClientContact: "updateClientContact"
			}),
			openGenders() {
			...
			useContactUpdate({ index, file, contact }) {
				// this.photoFile.push(file)
				this.updateActionClientContact({ index, contact })
				this.$router.go(-1)
			},
			contactUpdate() {
				this.useContactUpdate({ file: this.photoFile[0], index: this.index || this.$route.params.index, contact: this.contact })
			},
			getContact() {
