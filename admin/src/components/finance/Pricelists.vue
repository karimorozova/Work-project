      ...
      .table
        GeneralTable(
          ...
          template(slot="headerVendorDefault" slot-scope="{ field }")
            .table__header {{ field.label }}
          template(slot="headerActiveClient" slot-scope="{ field }")
            .table__header {{ field.label }}
          template(slot="headerActiveVendor" slot-scope="{ field }")
            .table__header {{ field.label }}
          ...
          template(slot="clientDefault" slot-scope="{ row, index }")
            .table__data.pricelists_centered(style="width: 100%; text-align: center;")
              CheckBox(
                :isChecked="row.isClientDefault"
...
export default {
  ...
  data() {
    return {
      pricelists: [],
      fields: [
        { label: "Name", headerKey: "headerName", key: "name", style: { width: "40%" } },
        { label: "Default Client", headerKey: "headerActiveClient", key: "clientDefault", style: { width: "15%" } },
        { label: "Default Vendor", headerKey: "headerVendorDefault", key: "vendorDefault", style: { width: "15%" } },
        { label: "Active", headerKey: "headerActiveVendor", key: "active", style: { width: "10%" } },
  ...
  methods: {
    ...
    async setDefaultPricelist(e, index, prop) {
      ...
      const exDefaultPriceId = currentDefaultPrice && currentDefaultPrice._id || this.pricelists.find(Boolean)._id
      try {
