...
const sendMailPortal = function(request) {
    var detailFile = "";
    if(request.genBrief) {
      detailFile = `<a href='http://testadmin.pangea.global/reqfiles/${request.id}/written.txt' download target='_self'>written.txt</a>`;
    }
    if (request.detailFiles.length > 0) {
      for (var i = 0; i < request.detailFiles.length; i++) {
        detailFile += "<a href='http://testadmin.pangea.global/reqfiles/" + request.id + "/";
    ...
    if (request.refFiles.length > 0) {
      for (var i = 0; i < request.refFiles.length; i++) {
        referenceFile += "<a href='http://testadmin.pangea.global/reqfiles/" + request.id + "/";
    ...
    let transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: '@Png2022!!' //fc72170d536b40480711bfad6ff1a8c1
      }
    });
