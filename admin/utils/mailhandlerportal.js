...
const sendMailPortal = function(request) {
    var detailFile = "";
    if(request.genBrief) {
      detailFile = `<a href='http://adminpangea.global/reqfiles/${request.id}/written.txt' download target='_self'>written.txt</a>`;
    }
    if (request.detailFiles.length > 0) {
      for (var i = 0; i < request.detailFiles.length; i++) {
        detailFile += "<a href='http://adminpangea.global/reqfiles/" + request.id + "/";
    ...
    if (request.refFiles.length > 0) {
      for (var i = 0; i < request.refFiles.length; i++) {
        referenceFile += "<a href='http://adminpangea.global/reqfiles/" + request.id + "/";
