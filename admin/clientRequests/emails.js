...
async function clientRequestNotification(request, prop) {
    try {
      const msg = prop ? managerRequestAssignedMessage({
        ...request._doc,
        user: request[prop]
...
async function sendNotificationToManager(request, prop) {
    try {
      ...
      await clientRequestNotification(request, prop);
    } catch(err) {
        console.log(err);
...
module.exports = { clientRequestNotification, sendNotificationToManager, notifyRequestCancelled };
