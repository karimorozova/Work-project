...
function messageForClientSendQuote(obj, allUnits, allSettingsSteps) {
	...
	let acceptQuote = '<a href=' + `${ apiUrl }/projectsapi/pangea-re-survey-page-acceptquote?projectId=${ obj.id }&to=${ date }&t=${ token }` + ` target="_blank" >I accept - ${ obj.projectId }, ${ total } ${ returnIconCurrencyByStringCode(obj.projectCurrency) }</a>`
	let declineQuote = '<a href=' + `${ apiUrl }/projectsapi/pangea-re-survey-page-declinequote?projectId=${ obj.id }&to=${ date }t=${ token }` + ` target="_blank" >I reject - ${ obj.projectId }, ${ total } ${ returnIconCurrencyByStringCode(obj.projectCurrency) }</a>`
	if (obj.selectedTasks.length) {
		...
		acceptQuote = '<a href=' + `${ apiUrl }/projectsapi/pangea-re-survey-page-accept-decline-tasks-quote?projectId=${ obj.id }&tasksIds=${ taskIdsString }&t=${ token }&to=${ date }&prop=Approved` + ` target="_blank" >I accept - ${ obj.projectId }, ${ +totalForSelected.toFixed(2) } ${ returnIconCurrencyByStringCode(obj.projectCurrency) }</a>`
		declineQuote = '<a href=' + `${ apiUrl }/projectsapi/pangea-re-survey-page-accept-decline-tasks-quote?projectId=${ obj.id }&tasksIds=${ taskIdsString }&t=${ token }&to=${ date }&prop=Rejected` + ` target="_blank" >I reject - ${ obj.projectId }, ${ +totalForSelected.toFixed(2) } ${ returnIconCurrencyByStringCode(obj.projectCurrency) }</a>`
	}
...
function getPdfOfQuote(allUnits, allSettingsSteps, obj, tasksIds = []) {
	...
	return `<div class="wrapper pdf" style="width:800px; margin: 0 auto; font-size:14px!important;font-family:'Roboto', sans-serif;color:#66563E;box-sizing:border-box;" >
                <header style="border: 1px solid #c1bbb1;text-align:center;padding-top:15px;padding-bottom:15px;padding-right:0;padding-left:0;" >
                ...
                <div class="quote" style=" border: 1px solid #c1bbb1; display: -webkit-box;" >
                ...
                <div class="quote__subheader" style="text-align:center;font-weight:bold;padding-top:15px;padding-bottom:15px;padding-right:0;padding-left:0;border: 1px solid #c1bbb1;" >
                    ${ obj.projectName }
                </div>
