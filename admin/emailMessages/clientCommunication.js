...
function messageForClientSendQuote(project, tasksIds, allUnits, allSettingsSteps) {
	const date = Date.now()
	const token = jwt.sign({ id: project._id }, secretKey, { expiresIn: '21d' })
	let { accountManager, tasks, steps, finance, discounts, projectCurrency } = project
	...
	const displaySubTotal = discounts.length || TMDiscount > 0.05
			? `<p style="padding: 5px 0; font-size: 14px; font-weight: 600;"> Sub-total: ${ subTotal } ${ returnIconCurrencyByStringCode(project.projectCurrency) }</p>`
			: ''

	const displaySubTotal2 = project.paymentAdditions.length
			? `<p style="padding: 5px 0; font-size: 14px; font-weight: 600;"> ${ discounts.length || TMDiscount ? 'Sub-total (2):' : 'Sub-total:' } ${ (total - project.paymentAdditions.reduce((a, c) => a + c.value, 0)).toFixed(2) } ${ returnIconCurrencyByStringCode(project.projectCurrency) }</p>`
			: ''
	...
	let acceptQuote = `<a href="${ acceptHref }" style="background: #47A6A6;color: #fff;padding: 6px 20px;text-decoration: none;border-radius: 4px;cursor: pointer;margin: 3px 10px 10px 0;display: inline-block;">I accept ${ total } ${ returnIconCurrencyByStringCode(projectCurrency) }</a>`
	let declineQuote = `<a href="${ declineHref }" style="background: #d15f45;color: #fff;padding: 6px 20px;text-decoration: none;border-radius: 4px;cursor: pointer;margin: 3px 10px 10px 0;display: inline-block;">I reject ${ total } ${ returnIconCurrencyByStringCode(projectCurrency) }</a>`
...
function getPdfOfQuote(project, tasksIds, allUnits, allSettingsSteps) {
	let { steps, finance, discounts, projectCurrency, projectId, clientBillingInfo } = project
	...
	<div class="pdf" style="width: 814px; font-size: 12px; font-family: Arial, sans-serif;color: #333;padding: 30px;position: relative;">
			...
			<div class="body" style="padding: 25px;margin-top: 50px; width: 693px;">
			...
			<div class="details" style="padding: 25px; width: 693px; margin-bottom: 70px;">
				${ getProjectDetails(project) }
				${ getJobsDetails(project, tasksIds, steps, allUnits) }
				${ isHideWhenMinimumCharge ? '' : displaySubTotal }
				${ isHideWhenMinimumCharge ? '' : displayFinanceDetails }
			...
			<div class="footer" style="position: absolute;width: 693px;border-top: 3px solid #c8e4e4;margin-left: 25px;bottom: 0;padding: 15px 0 12px;">
...
function messageForClientSendCostQuote(project, allUnits, allSettingsSteps) {
	const tasksIds = []
	let { accountManager, tasks, steps, finance, discounts, projectCurrency } = project
	steps = steps.filter(item => item.status !== "Cancelled")
	const subTotal = getStepsSubTotal(tasksIds, steps, allUnits)
...
async function pdfPPPReportTemplate(report, BI) {
	...
	const displaySubTotal2 = project.paymentAdditions.length
			? `<p style="padding: 5px 0; font-size: 14px;  text-align: right; font-weight: 600;"> ${ discounts.length || TMDiscount ? 'Sub-total (2):' : 'Sub-total:' } ${ (total - project.paymentAdditions.reduce((a, c) => a + c.value, 0)).toFixed(2) } ${ returnIconCurrencyByStringCode(project.projectCurrency) }</p>`
			: ''
